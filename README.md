...
Этот репозиторий представляет собой **универсальный boilerplate** для развертывания Laravel-проектов с использованием связки сервисов через **TCP**.

## 🚀 Основные возможности

*   **Высокая производительность:** Связь между Httpd и PHP-FPM реализована через **TCP (порт 9000)**, что обеспечивает стабильную и простую работу в Docker-окружении без необходимости шарить файловую систему для сокета.
*   **Современный стек:**
    *   **PHP 8.5 (Alpine)** — свежая версия с предустановленными расширениями (bcmath, gd, intl, pdo_pgsql и др.).
    *   **Httpd (Apache)** — оптимизированный конфиг для Laravel на `mod_proxy_fcgi`.
...
*   `make up` — Запустить проект в dev-режиме (на порту `8050` или `HTTPD_PORT`).
