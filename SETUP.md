...
* Файлы `docker-compose.yml`, `docker-compose.dev.yml`, `docker-compose.prod.yml`, `docker-compose.prod.local.yml`
* Файл `Makefile`
...
# на имя сервиса БД из вашего docker-compose.yml (по умолчанию: laravel-postgres-apache-tcp)
# А REDIS_HOST=127.0.0.1 на laravel-redis-apache-tcp
...
**Важно:** 
1. В секции БД (в начале `.env`) обязательно замените `DB_HOST=127.0.0.1` на имя сервиса из `docker-compose.yml` (например, `laravel-postgres-apache-tcp`).
