...
Этот boilerplate предназначен для быстрого развертывания Laravel-проекта с архитектурой **PHP-FPM 8.5 + Apache Httpd 2.4 (TCP) + PostgreSQL 18.2 + Redis 8.6**.
...
Apache Httpd выступает как frontend web server и проксирует PHP-запросы в PHP-FPM через TCP (`laravel-php-apache-tcp:9000`). Это упрощает контейнеризацию и избавляет от необходимости пробрасывать Unix socket между контейнерами.
...
- Проверьте `DB_HOST=laravel-postgres-apache-tcp`
- Проверьте `make logs-postgres`
- Убедитесь, что в `.env` выбран `DB_CONNECTION=pgsql`

### Redis недоступен
- Проверьте `REDIS_HOST=laravel-redis-apache-tcp`
